...
impl<S, N> FormatEvent<S, N> for SovdCoreFormatter
where
    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(
        ...
        let lvl_style = SovdCoreFormatter::style_for(*meta.level());
        ...
        if meta.name() != "log event"
            && !(meta.name().starts_with("event sovd-") && meta.name().contains("/src/"))
        {
